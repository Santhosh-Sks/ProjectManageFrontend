...
import { FaChevronDown, FaChevronUp, FaReply, FaEdit, FaTrash, FaSmile } from 'react-icons/fa';
...
const CommentList = ({ taskId }) => {
        ...
        socketRef.current.on('reactionAdded', ({ commentId, reaction, user }) => {
            setComments(prev => prev.map(comment => {
                if (comment.id === commentId) {
                    ...
                    if (!reactions[reaction].includes(user)) reactions[reaction].push(user);
    ...
    const fetchTeamMembers = async () => {
        try {
            const response = await api.get(`/api/projects/${taskId}/members`);
    ...
    const handleReaction = async (commentId, reaction) => {
        try {
            await api.post(`/api/comments/${commentId}/reactions`, {
                reaction,
                user: user.email
            });
                ...
                {editingComment === comment.id ? (
                    <div className="edit-comment">
                        ...
                        <div className="edit-actions">
                            <button onClick={() => handleEdit(comment.id, newComment)} className="btn btn-secondary">Save</button>
                            <button onClick={() => { setEditingComment(null); setNewComment(''); }} className="btn btn-link">Cancel</button>
                        </div>
                    </div>
                ) : (
                    <>
                        <div className="comment-header">
                            ...
                            {user.email === comment.createdBy && (
                                <div className="comment-actions">
                                    <button onClick={() => { setEditingComment(comment.id); setNewComment(comment.content); }} className="btn btn-link"><FaEdit /></button>
                                    <button onClick={() => handleDelete(comment.id)} className="btn btn-link text-danger"><FaTrash /></button>
                                </div>
                            )}
                        </div>
                        <p className="comment-content">{comment.content}</p>
                        <div className="comment-reactions">
                            <button className="btn btn-link" onClick={() => setShowReactions(showReactions === comment.id ? null : comment.id)}><FaSmile /></button>
                            {showReactions === comment.id && (
                                <div className="reactions-picker">
                                    {Object.entries(REACTIONS).map(([emoji, name]) => (
                                        <button key={emoji} className="reaction-btn" onClick={() => handleReaction(comment.id, emoji)} title={name}>{emoji}</button>
                                    ))}
                                </div>
                            )}
                            {comment.reactions && Object.entries(comment.reactions).map(([emoji, users]) =>
                                users.length > 0 && (
                                    <div key={emoji} className="reaction-badge" title={users.join(', ')}>
                        ...
                        <div className="comment-actions">
                            <button onClick={() => handleReply(comment.id)} className="btn btn-link"><FaReply /> Reply</button>
                            {replyCount > 0 && (
                                <button onClick={() => toggleThread(comment.id)} className="btn btn-link">
                                    {isCollapsed ? <FaChevronDown /> : <FaChevronUp />} {replyCount} {replyCount === 1 ? 'reply' : 'replies'}
                                </button>
                            )}
                        </div>
    ...
    return (
        <div className="comment-section">
            ...
            <form onSubmit={handleSubmit} className="comment-form">
                ...
                />
                {showMentions && (
                    <div className="mentions-dropdown" style={{ top: mentionPosition.top, left: mentionPosition.left }}>
                        {teamMembers
                            .filter(member => member.email.toLowerCase().includes(mentionSearch.toLowerCase()))
                            .map(member => (
                                <div key={member.email} className="mention-item" onClick={() => handleMention(member)}>
                                    {member.email}
                                </div>
                            ))}
                    </div>
                ...
                <div className="form-actions">
                    {replyingTo && (
                        <button type="button" onClick={() => setReplyingTo(null)} className="btn btn-link">Cancel Reply</button>
                    )}
                    <button type="submit" className="btn">{replyingTo ? 'Post Reply' : 'Add Comment'}</button>
                </div>
            </form>
