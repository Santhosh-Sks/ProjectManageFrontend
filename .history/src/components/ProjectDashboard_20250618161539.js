...
import { FaTasks, FaUsers, FaEnvelope, FaPlus, FaTrash, FaEdit } from 'react-icons/fa';
...
const ProjectDashboard = () => {
    ...
    const handleTaskSubmit = async (e) => {
        e.preventDefault();
        try {
            const response = await axios.post(`http://localhost:8080/api/projects/${projectId}/tasks`, newTask);
    ...
    const handleInviteSubmit = async (e) => {
        e.preventDefault();
        try {
            await axios.post(`http://localhost:8080/api/projects/${projectId}/invite`, { email: inviteEmail });
            setInviteEmail('');
            setShowInviteForm(false);
            // Refresh team members list
            const membersResponse = await axios.get(`http://localhost:8080/api/projects/${projectId}/members`);
            setTeamMembers(membersResponse.data);
        } catch (err) {
    ...
    const handleTaskStatusChange = async (taskId, newStatus) => {
        try {
            await axios.patch(`http://localhost:8080/api/tasks/${taskId}`, { status: newStatus });
