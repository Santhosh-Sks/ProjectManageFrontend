...
export const taskService = {
    ...
    addComment: async (taskId, comment) => {
        try {
            // First get the current task data to preserve all existing fields
            const currentTask = await authenticatedApi.get(`${API_URL}/tasks/${taskId}`);
            console.log('Current task data:', currentTask.data);
            
            // Prepare the update data
            const updateData = { 
                ...currentTask.data, // Preserve all existing task data
                comment: comment,
                commentAddedAt: new Date().toISOString()
            };
            console.log('Sending update data:', updateData);
            
            // Update the task with all existing data plus the new comment
            const response = await authenticatedApi.put(`${API_URL}/tasks/${taskId}`, updateData);
