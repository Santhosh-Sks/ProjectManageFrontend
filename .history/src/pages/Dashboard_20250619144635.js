...
const Dashboard = () => {
    ...
    const loadDashboardData = useCallback(async () => {
        try {
            ...
            // Store sorted for use in render
            window._sortedUserProjects = sorted;
    ...
    return (
        <div className="dashboard-container">
            ...
            <div className="dashboard-main-grid">
                <div className="dashboard-panel" style={{ minHeight: '260px' }}>
                    <h2>Most Recent Projects</h2>
                    {window._sortedUserProjects && window._sortedUserProjects.length > 0 ? (
                        <RecentProjects projects={window._sortedUserProjects.slice(0, 3)} />
                    ) : (
            ...
            <div className="projects-grid">
                {projects
                    ...
                    .filter(project =>
                        !(window._sortedUserProjects && window._sortedUserProjects.slice(0, 3).some(
                            p => (p.id || p._id) === (project.id || project._id)
                        ))
                    )
                    .map(project => (
