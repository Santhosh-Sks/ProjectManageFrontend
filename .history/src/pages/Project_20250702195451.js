...
const Project = () => {
    ...
    const loadCommentsForTask = useCallback(async (taskId) => {
        try {
            const response = await fetch(`${API}/api/comments/task/${taskId}`, {
                headers: {
    ...
    const loadTasksWithComments = useCallback(async () => {
        try {
            setError(null);
            const data = await taskService.getTasksByProjectId(id);
    ...
    }, [id, loadCommentsForTask]);

    useEffect(() => {
        loadProject();
        loadTasksWithComments();
    }, [loadProject, loadTasksWithComments]);
                ...
                <div className="members-section">
                    ...
                    <form
                        onSubmit={async (e) => {
                            ...
                            setInviteMsg("");
                            try {
                                const res = await fetch(`${API}/api/emails/invitation`, {
                                    method: 'POST',
                                            ...
                                            {task.comments?.length > 0 ? (
                                                task.comments.map((comment, index) => (
                                                    <li key={comment.id || index}>
                                                        <p>{comment.content || comment.text || comment.message || comment.comment || JSON.stringify(comment)}</p>
