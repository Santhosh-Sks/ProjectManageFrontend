...
import SSLError from './pages/SSLError';
import './App.css';
...
const AppContent = () => {
  ...
  useEffect(() => {
    ...
    window.addEventListener('error', (event) => {
      if (event.message.includes('SSL') || event.message.includes('protocol')) {
        handleSSLError();
      }
    });
    ...
    return () => {
      window.removeEventListener('error', handleSSLError);
    };
  }, []);
